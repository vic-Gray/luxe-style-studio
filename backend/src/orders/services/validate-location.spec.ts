...
describe('validateLocation()', () => {
  ...
  it('returns sanitized coords for a typical Lagos location', () => {
    ...
    expect(validateLocation(raw, logger)).toEqual({ lat: 6.5244, lng: 3.3792, accuracy: 35 });
    expect(warnSpy).not.toHaveBeenCalled();
  });

  it('accepts accuracy = 0', () => {
    const raw: LocationDto = { lat: -33.8688, lng: 151.2093, accuracy: 0 };
    expect(validateLocation(raw, logger)).toEqual({ lat: -33.8688, lng: 151.2093, accuracy: 0 });
  });

  it('returns accuracy: null when accuracy is omitted', () => {
    expect(validateLocation({ lat: 6.5244, lng: 3.3792 }, logger))
      .toEqual({ lat: 6.5244, lng: 3.3792, accuracy: null });
  });
  ...
  it('rejects string lat', () => {
    const bad = { lat: '6.5244' as unknown as number, lng: 3.3792 } as LocationDto;
    expect(validateLocation(bad, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it('rejects string lng', () => {
    const bad = { lat: 6.5244, lng: '3.3792' as unknown as number } as LocationDto;
    expect(validateLocation(bad, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it('rejects negative accuracy', () => {
    expect(validateLocation({ lat: 6.5244, lng: 3.3792, accuracy: -1 }, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });

  it('rejects NaN accuracy', () => {
    expect(validateLocation({ lat: 6.5244, lng: 3.3792, accuracy: NaN }, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });

  it('rejects Infinity accuracy', () => {
    expect(validateLocation({ lat: 6.5244, lng: 3.3792, accuracy: Infinity }, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });

  it('rejects string accuracy', () => {
    const bad = { lat: 6.5244, lng: 3.3792, accuracy: 'bad' as unknown as number } as LocationDto;
    expect(validateLocation(bad, logger)).toBeNull();
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it('ignores extra keys when coords are valid', () => {
    const raw = { lat: 6.5244, lng: 3.3792, accuracy: 35, extra: 'junk' } as LocationDto;
    expect(validateLocation(raw, logger)).toEqual({ lat: 6.5244, lng: 3.3792, accuracy: 35 });
    expect(warnSpy).not.toHaveBeenCalled();
  });
